...
const create = (req, res) => {
  ...
  if (!categoryType)
    return res.status(400).json({ message: "Tipo da categoria está undefined!" })

  categoryModel.create(userId, categoryName, categoryType)
  .then(() => {
    categoryModel.getLatestByUser(userId)
...
const update = (req, res) => {
  const { userId, categoryId, categoryName, categoryType } = req.body
  ...
  categoryModel.update(userId, categoryId, categoryName, categoryType)
